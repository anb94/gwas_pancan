...
for ((i=2; i<=22; i++)); do
    ...
    cat "${WHI_SHARE_ha_cb}"/SHAREchr"${i}"aa_lq03_snps.txt >> "${WHI_SHARE_ha_cb}"/SHARE_ha_lq03_snps.txt
done

## Append the low qual 0.8 snp files to a combined file
for ((i=2; i<=22; i++)); do
    echo "Doing SHARE_ha lq08 ${i}"
    cat "${WHI_SHARE_ha_cb}"/SHAREchr"${i}"aa_lq08_snps.txt >> "${WHI_SHARE_ha_cb}"/SHARE_ha_lq08_snps.txt
done